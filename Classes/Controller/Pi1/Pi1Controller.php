...
class tx_staffdirectory_pi1 extends Tx_StaffDirectory_Controller_AbstractController {
	...
	protected function listAction() {
		...
		if ($this->conf['staffs']) {
			$uids = t3lib_div::intExplode(',', $this->conf['staffs']);
			$staffs = array();
			foreach ($uids as $uid) {
	...
	protected function renderMember($context, $template, Tx_StaffDirectory_Domain_Model_Staff $staff = NULL, Tx_StaffDirectory_Domain_Model_Member $member = NULL, tslib_cObj $contentObj) {
		...
		$subparts = array();
			...
			// Hide presentation parts if empty
		if (!$member->getPositionFunction()) {
			$subparts['###IF_POSITION_FUNCTION###'] = '';
		}
		if (!$member->getAddress()) {
			$subparts['###IF_ADDRESS###'] = '';
		}
		if (!$member->getTelephone()) {
			$subparts['###IF_TELEPHONE###'] = '';
		}
		if (!$member->getFax()) {
			$subparts['###IF_FAX###'] = '';
		}
		if (!$member->getMobilePhone()) {
			$subparts['###IF_MOBILE_PHONE###'] = '';
		}
		if (!$member->getEmail()) {
			$subparts['###IF_EMAIL###'] = '';
		}
		...
		return $this->render($template, $member->toArray(), $contentObj, $tsConfig, $markers, $subparts);
	}
	...
	protected function getTypolinkConf($uid, array $params) {
		$conf = array(
			...
			'useCacheHash' => 1,
			'no_cache' => 0,
			'additionalParams' => t3lib_div::implodeArrayForUrl($this->prefixId, $params, '', TRUE),
		);
		return $conf;
	}
	...
	protected function init(array $settings) {
		...
		$dao = t3lib_div::makeInstance('Tx_StaffDirectory_Persistence_Dao', $this->conf, $this->cObj);
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/staffdirectory/Classes/Controller/Pi1/Pi1Controller.php'])) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/staffdirectory/Classes/Controller/Pi1/Pi1Controller.php']);
}
